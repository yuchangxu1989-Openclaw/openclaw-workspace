...
// ============================================================================
// CapabilityAnchor Types
// ============================================================================

interface CapabilityAnchorData {
  models: Record<string, ModelCapability>;
  routingHints?: {
    intentToModel?: Record<string, string>;
  };
  metadata?: {
    version: string;
...
export class PreferenceMerger {
  private capabilityAnchor: CapabilityAnchorData | null = null;
  private agentConfigs: Map<string, AgentConfig> = new Map();
  private anchorPath: string;
  private anchorCacheTime: number = 0;
  private readonly ANCHOR_CACHE_TTL = 30000; // 30秒缓存
  ...
  async merge(
    ...
    let capabilityMatches: CapabilityMatch[] = [];

    if (enforceCapabilityMatch && this.capabilityAnchor) {
      const filterResult = this.filterByCapability(
        preferenceChain,
        intent,
        minMatchScore
      );
  ...
  private checkIntentOverride(
    ...
    if (!agentConfig.intentOverrides) return null;
    ...
    if (override.minComplexity) {
      ...
      if (intentLevel < requiredLevel) return null;
    }
  ...
  private buildOverrideChain(
    ...
    if (!agentConfig.modelPreferences.strictMode) {
      const fallbackChain = agentConfig.modelPreferences.fallbacks.filter(
        m => m !== override.preferredModel
      );
   ...
   * 构建偏好链
   */
  private buildPreferenceChain(preferences: ModelPreference): string[] {
  ...
  ): { filtered: string[]; matches: CapabilityMatch[] } {
  ...
  private calculateCapabilityMatch(
    ...
    const intentComplexityMap: Record<string, string> = {
      'low': 'L1',
      'medium': 'L2',
      'high': 'L3',
      'extreme': 'L5'
    ...
    if (model.capabilities.some(c => 
      c.includes(domainCapability) || c.includes(intent.taskCategory)
    )) {
      totalScore += 1;
   ...
   */
  private recommendFromCapabilityAnchor(intent: TaskIntent): string[] {
    if (!this.capabilityAnchor) return ['{{MODEL_GENERAL}}'];

    const recommendations: Array<{ model: string; score: number }> = [];
  ...
  private adjustByComplexity(
    chain: string[],
    complexity: TaskIntent['complexity']
  ): string[] {
    if (!this.capabilityAnchor) return chain;

    // 复杂度优先级映射
    const complexityPriority: Record<string, string[]> = {
   ...
   * 领域到能力的映射
   */
  private mapDomainToCapability(domain: string): string {
   ...
   * 加载CapabilityAnchor
   */
  private loadCapabilityAnchor(): void {
    try {
      // 检查缓存是否有效
      const now = Date.now();
      ...
      for (const tryPath of possiblePaths) {
        if (fs.existsSync(tryPath)) {
          ...
          this.capabilityAnchor = yaml.load(content) as CapabilityAnchorData;
  ...
  ): Record<string, IntentOverride> | undefined {
    if (!rawOverrides) return undefined;

    const overrides: Record<string, IntentOverride> = {};
  ...
  getAvailableModels(): string[] {
    if (!this.capabilityAnchor) return [];
  ...
  getModelCapability(modelPlaceholder: string): ModelCapability | null {
    if (!this.capabilityAnchor) return null;
