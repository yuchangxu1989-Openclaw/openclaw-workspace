...
## 🚫 主Agent禁止操作清单（7种逃逸路径封堵，2026-03-08）

> 根因：委派铁律只有"不该做"的定性描述，缺少对具体逃逸路径的量化封堵，导致铁律形同虚设。

以下7种操作模式，主Agent**绝对禁止**直接执行，必须委派子Agent：

| # | 逃逸路径 | 判定条件 | 处置 |
|---|---------|---------|------|
| 1 | 连续exec | 同一用户消息响应中exec调用≥3次 | 立即停止，委派子Agent |
| 2 | 修改型shell命令 | sed -i / tee / > / >> / awk写文件，即使1次 | 委派子Agent |
| 3 | feishu_doc写操作 | feishu_doc write / append，即使1次 | 委派子Agent |
| 4 | 多行exec | 单次exec命令体超过3行 | 委派子Agent |
| 5 | exec调用脚本做实现 | exec运行自定义脚本（非读取/验证类）做实现性工作 | 委派子Agent |
| 6 | browser实现性自动化 | 用browser工具做实现性页面操作（非验证/截图） | 委派子Agent |
| 7 | 大量read后直接输出 | read超过5个文件后直接在回复中输出完整方案 | 委派子Agent做分析思考 |

**自检口诀：** 读可以，改不行；验可以，做不行；判断可以，实现不行。
