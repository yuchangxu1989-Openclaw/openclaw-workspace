...
interface IntentTemplate {
  ...
  category: string;
  description: string;
  semantic_vectors: Record<string, SemanticVector>;
  complexity_indicators: Record<string, ComplexityIndicator>;
  input_modality_detection?: Record<string, ModalityDetection>;
  default_model: string;
  fallback_chain: string[];
  routing_hints: RoutingHints;
  quick_response_patterns?: string[];
}
...
export class IntentClassifier {
  ...
  private templates: Map<string, IntentTemplate> = new Map();
  private cache: Map<string, TaskIntent> = new Map();
  private cacheTTL: number = 60000; // 60秒缓存
   ...
   * 将新JSON格式转换为内部格式
   */
  private convertToInternalFormat(raw: any, category: string): IntentTemplate {
    const semanticVectors: Record<string, SemanticVector> = {};
    const complexityIndicators: Record<string, ComplexityIndicator> = {};
      ...
      for (const intent of raw.intents) {
        ...
        complexityIndicators[complexityLevel] = {
          ...
          max_tokens_estimate: intent.task_examples ? 
            intent.task_examples.reduce((max: number, ex: string) => Math.max(max, ex.length * 2), 100) : 100
        };
      }
    }
    ...
    return {
      ...
      quick_response_patterns: raw.intents?.map((i: any) => i.semantic_features?.patterns || []).flat()
    };
  }
   ...
   * 解析复杂度范围字符串
   */
  private parseComplexityRange(range?: string): string {
    if (!range) return 'medium';
    if (range.includes('L5') || range.includes('L4')) return 'extreme';
    if (range.includes('L3')) return 'high';
    if (range.includes('L2')) return 'medium';
    return 'low';
  }
   ...
   */
  async classify(request: ClassificationRequest): Promise<TaskIntent> {
    ...
    // 2. 基于语义相似度分类
    const { category, semanticScores, confidence } = this.classifyBySemantics(
      request.description,
      request.context
    );
    ...
    const intent: TaskIntent = {
      taskCategory: category as 'reasoning' | 'multimodal' | 'general',
      complexity,
      inputModality,
      outputModality,
      domain,
  ...
  private classifyBySemantics(
    description: string,
    context?: TaskContext[]
  ): { category: string; semanticScores: Record<string, number>; confidence: number } {
   ...
   * 检测输入模态
   */
  private detectInputModality(request: ClassificationRequest): TaskIntent['inputModality'] {
    const modalities: string[] = [];

    // 检查附件
    if (request.attachments) {
      for (const attachment of request.attachments) {
        if (attachment.type === 'image') modalities.push('image');
        else if (attachment.type === 'audio') modalities.push('audio');
        else if (attachment.type === 'video') modalities.push('video');
      }
    }
    ...
    for (const url of urls) {
      ...
      if (/\.(jpg|jpeg|png|gif|webp)$/.test(lowerUrl)) modalities.push('image');
      else if (/\.(mp3|wav|ogg|m4a)$/.test(lowerUrl)) modalities.push('audio');
      else if (/\.(mp4|webm|avi)$/.test(lowerUrl)) modalities.push('video');
    }
    ...
    if (uniqueModalities.length === 0) return 'text';
    if (uniqueModalities.length === 1) return uniqueModalities[0] as TaskIntent['inputModality'];
    return 'mixed';
  }
  ...
  private detectComplexity(
    ...
    // 按复杂度顺序检测（从高到低）
    const levels: Array<TaskIntent['complexity']> = ['extreme', 'high', 'medium', 'low'];
    
    for (const level of levels) {
      const indicator = template.complexity_indicators[level];
      if (!indicator) continue;

      const matches = indicator.patterns.filter(p => 
        normalizedDesc.includes(this.normalizeText(p))
      ).length;
    ...
    if (length > 5000 && detectedLevel === 'low') return 'medium';
    if (length > 10000 && detectedLevel !== 'extreme') return 'high';

    return detectedLevel as TaskIntent['complexity'];
  }

  /**
   * 检测输出模态
   */
  private detectOutputModality(description: string): TaskIntent['outputModality'] {
   ...
   * 识别领域
   */
  private detectDomain(description: string, category: string): string {
  ...
  private adjustConfidence(
    ...
    if (!context || context.length === 0) return baseConfidence;
   ...
   * 快速分类（用于上下文分析）
   */
  private quickClassify(text: string): string {
   ...
   * 获取缓存
   */
  private getCached(key: string): TaskIntent | null {
   ...
   * 设置缓存
   */
  private setCached(key: string, intent: TaskIntent): void {
