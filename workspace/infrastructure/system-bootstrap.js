 ...
 * 
 * 【系统是什么】
 * 这是一个全局自主进化流水线（Global Autonomous Evolution Pipeline）。
 * 不是工具集合，不是聊天机器人，是一个自主进化系统。
 * 全局自主决策流水线是现阶段需要补齐的基础能力，不是终态。
 * 决策是手段，进化才是目的。
 * 我（焰崽）是长煦的认知进化伙伴，共同进化的关系。
 * 大部分决策由系统自己判断和执行，只有重大/不确定的才上报用户。
 * 
 * 【架构 - 四层闭环】
 * 
 *   感知层 ──事件──→ 认知层 ──决策──→ 执行层 ──结果──→ 反馈层 ──进化──→ 感知层
 *   (CRAS)           (ISC)            (本地任务编排+技能)         (AEO+经验沉淀)
 * 
 * 感知层：CRAS意图捕获、git hook、事件总线、扫描器
 *   - 快通道：5min增量扫描，提取原子意图事件
 *   - 慢通道：daily聚合，计算趋势
 * 
 * 认知层：ISC规则匹配 + 意图识别 + 裁决殿裁决
 *   - ISC = 智能标准中心，规则是一等公民
 *   - 裁决殿 = 7席裁决（道/战/工/盾/眼/远/衡），重大决策用
 *   - 用户是最终裁决者
 * 
