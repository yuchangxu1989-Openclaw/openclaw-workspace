...
- **结果**: FAIL
- **失败断点**: Step 3
  - 期望: `{"expect":"dispatched","handler_pattern":"*"}`
  - 原因: No handler dispatched
- **步骤执行**:
...
- **结果**: FAIL
- **失败断点**: Step 3
  - 期望: `{"expect":"dispatched","handler_pattern":"cras*"}`
  - 原因: Handler "error: Cannot read properties of undefined (reading 'startsWith')" doesn't match pattern "cras*"
- **步骤执行**:
  ✅ Step 0: Emitted user.message
  ✅ Step 1: Intent detected: {"category":"IC3","name":"academic_analysis","confidence":0.7,"source":"regex_fallback"}
  ✅ Step 2: 2 rules matched
  ❌ Step 3: Handler "error: Cannot read properties of undefined (reading 'startsWith')" doesn't match pattern "cras*"
