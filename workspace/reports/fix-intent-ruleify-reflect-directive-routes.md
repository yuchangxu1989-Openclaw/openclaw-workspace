...
- `intent.ruleify`
  ...
  - 额外发出 `isc.rule.created`，让后续 ISC/DTO 链路可继续消费
- `intent.reflect`
...
3. `intent.directive` 会进入 DTO task create bridge
...
✅ intent.directive routes into DTO task creation bridge
```
