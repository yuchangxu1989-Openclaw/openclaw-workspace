...
| 4 | **循环事件链**：DTO bridge中 isc.rule→dto.sync→seef.evaluated→aeo.completed→cras.insight→isc.rule.updated 形成环 | 事件雪崩，maxDepth仅控制单次dispatch | ❌ 未识别 |
...
当前存在两套总线API：
- `bus.js`：原始实现，SEEF/AEO/DTO/ISC直接使用
...
// 迁移顺序：ISC → DTO → AEO → SEEF（按耦合度从低到高）
...
| Phase 4 | DTO event-bridge改用bus-adapter | 中（DTO是最复杂的bridge） |
