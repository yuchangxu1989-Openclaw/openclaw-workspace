...
| PB-002 | easy | ✅ | 2ms | skill.updated → match but condition fails (no new_capability |
| PB-003 | easy | ✅ | 1ms | execution_failed → match but condition evaluates false |
| PB-004 | easy | ✅ | 2ms | design.document.created → 3 rules fire (arch-review, anti-en |
| PB-005 | easy | ✅ | 7ms | isc.rule.created → anti-entropy + layered-decoupling rules f |
| PB-006 | easy | ✅ | 1ms | evomap.sync.request → security scan rule fires |
| PB-007 | easy | ✅ | 0ms | message.reply.received → match but condition fails |
...
| PB-011 | easy | ✅ | 0ms | Unknown event → no matches, safe skip |
| PB-012 | easy | ✅ | 3ms | Noise event → zero matches |
...
| PB-019 | medium | ✅ | 2ms | Batch: 3 matchable events → 6+ total rule matches |
| PB-020 | medium | ✅ | 6ms | Batch: 3 events → 5+ rule matches |
...
| PB-023 | medium | ✅ | 3ms | user.message → IntentScanner triggered |
| PB-024 | medium | ✅ | 1ms | conversation.new → intent scan triggered |
| PB-025 | medium | ✅ | 2ms | chat.incoming → intent scan triggered |
...
| PB-031 | hard | ✅ | 3ms | 5-event batch: 4 matchable + 1 noise → 7+ rule matches |
| PB-032 | hard | ✅ | 0ms | All events exceed depth → all circuit-broken |
| PB-033 | hard | ✅ | 1ms | Conversation at high depth → circuit break, no intent scan |
| PB-034 | medium | ✅ | 1ms | L3_PIPELINE_ENABLED=false → entire pipeline skipped |
...
| PB-038 | easy | ✅ | 2ms | api_key_rate_limit → match but condition fails evaluation |
