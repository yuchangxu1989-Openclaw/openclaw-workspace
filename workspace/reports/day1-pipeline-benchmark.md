...
|----|------|--------|----------|----------|
| PB-001 | easy | ✅ | 15ms | skill.created → matches 2+ ISC rules (anti-entropy, layered- |
...
| PB-005 | easy | ✅ | 6ms | isc.rule.created → anti-entropy + layered-decoupling rules f |
| PB-006 | easy | ✅ | 3ms | evomap.sync.request → security scan rule fires |
| PB-007 | easy | ✅ | 2ms | message.reply.received → match but condition fails |
| PB-008 | easy | ✅ | 2ms | skill.publish → security gate rule fires |
...
| PB-018 | easy | ✅ | 1ms | depth=0 → normal processing |
| PB-019 | medium | ✅ | 2ms | Batch: 3 matchable events → 6+ total rule matches |
| PB-020 | medium | ✅ | 7ms | Batch: 3 events → 5+ rule matches |
...
| PB-023 | medium | ✅ | 16764ms | user.message → IntentScanner triggered |
| PB-024 | medium | ✅ | 9560ms | conversation.new → intent scan triggered |
| PB-025 | medium | ✅ | 14206ms | chat.incoming → intent scan triggered |
...
| PB-033 | hard | ✅ | 0ms | Conversation at high depth → circuit break, no intent scan |
