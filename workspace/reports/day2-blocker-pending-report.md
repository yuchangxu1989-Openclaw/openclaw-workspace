...
**根本原因：** routes.json 里的别名映射（`lto-sync` → `skill-lto-handler`）在旧版本中，dispatcher用的是原始handler名（`lto-sync`），而 `handlers/` 目录里没有 `lto-sync.js` 文件。结果每次有 `isc.rule.*` 事件，都会写一个 `pending_execution` 文件，但没有任何东西消费它们。
...
2. **handler可执行化**: 当前 `lto-sync`、`seef-optimize`、`cras-ingest`、`isc-feedback` 等handler只有routes别名，没有实际执行逻辑。后续如需真正执行，需要在 `handlers/` 目录补充对应JS文件
