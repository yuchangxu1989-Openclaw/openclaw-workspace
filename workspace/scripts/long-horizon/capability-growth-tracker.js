...
const metrics = {
  ...
  dimensions: {
    knowledge: {
      // 知识沉淀维度：规则数、MemOS记忆条数
      rule_files: countFiles('skills', 'md') + countFiles('skills', 'json'),
      memory_lines: (() => { try { const m = require('/root/.openclaw/workspace/scripts/memos-reader'); const s = m.getStats(); if (s.activeChunks > 0) return s.activeChunks; } catch {} return 0; })(),
