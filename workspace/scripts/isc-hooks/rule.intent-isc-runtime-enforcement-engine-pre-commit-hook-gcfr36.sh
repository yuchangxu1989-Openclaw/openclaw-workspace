#!/usr/bin/env bash
set -euo pipefail
RULE_ID="rule.intent-isc-runtime-enforcement-engine-pre-commit-hook-gcfr36"
RULE_FILE="skills/isc-core/rules/rule.intent-isc-runtime-enforcement-engine-pre-commit-hook-gcfr36.json"
DETAIL="ok"
STATUS="pass"
if [ ! -f "$RULE_FILE" ]; then
  STATUS="fail"
