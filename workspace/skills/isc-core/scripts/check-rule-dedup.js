...
// =============================================================================
// 三维度语义去重：event交集快筛 → LLM深检condition+action等价性
//
// 用法:
...
async function callLLM(prompt) {
  const result = await llmContext.chat(
    ...
    { capability: 'chat', priority: 'cost', timeout: 30_000, maxTokens: 1024 }
  );
...
async function deepCheck(newRule, existingRule, verbose) {
  ...
  if (verbose) {
    console.error(C.dim(`  [LLM] 调用 LLM 判断 ${existingRule.id || path.basename(existingRule._file || '')}...`));
  }
...
async function main() {
  ...
  // ── 阶段二：LLM深检 ──────────────────────────────────────────────
  console.log(C.bold('【阶段二】LLM深检 — condition + action语义等价判断'));
  console.log('─────────────────────────────────────────────────────────');
