...
去重流程：扫描 `rules/` → 提取三维向量 → 两两比对 → 输出重复度矩阵 → 给出合并建议。

### 二、命名统一

强制格式：`rule.{domain}-{name}-{seq}.json`
- 文件名必须 `rule.` 前缀
- domain 在已知域列表中
- seq 为三位数字
- 非标命名输出重命名映射表
...
输出规则-事件-DTO对齐矩阵，识别每条规则在感知层/认知层/执行层的覆盖状态。

### 四、缺口扫描

按领域（quality/naming/pipeline/security/architecture）统计规则覆盖，识别无规则覆盖的关键事件，输出缺口报告。

---

## 规则化方法论（合并自 ruleify 技能）

**核心原则：没有代码的规则 = 不存在。规则化 = 交付可运行的自动化链条。**
...
- **感知层**：探针代码（git hook / 文件watcher / cron扫描 / 事件总线监听）
- **认知层**：判断逻辑（条件检查函数 / 规则匹配 / LLM辅助判断）
- **执行层**：动作代码（自动修复 / 阻断操作 / 通知告警）
