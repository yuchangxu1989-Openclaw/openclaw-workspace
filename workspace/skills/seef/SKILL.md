...
SEEF 是技能治理的**基础设施层能力**，由**八个独立子技能**构成。每个子技能均为自治单元，拥有：
- 唯一功能边界
- 明确定义的输入输出契约
- 与 ISC 和 本地任务编排 的标准化交互协议

**运行模式**：
- 可被 本地任务编排 按预设流程调用（如每日凌晨 2:00 执行全量进化）
...
| **对外** | 向 本地任务编排 与大模型提供标准化接口，支持固定流程调度或高自由度混合编排 |
| **对齐** | 所有子技能的准入与准出，必须显式依赖 ISC 提供的规范阈值，并响应 本地任务编排 的触发指令 |
...
| 8 | 技能安装器 | skill-installer | 封装ClawHub CLI，提供技能搜索/安装/更新能力 |
...
- 必须向 本地任务编排 注册本次调用上下文（trace_id, caller_type: llm）

### 2. 固定闭环模式

由 本地任务编排 调用主入口 `seef.run(mode='fixed')`，自动按顺序执行：
...
2. **本地任务编排 提供的调度许可**（显式指令或隐式策略）

> ⚠️ **任何子技能不得绕过 ISC 标准或 本地任务编排 调度直接流转，杜绝"黑箱"**

## 依赖与边界声明

| 项目 | 说明 |
|:-----|:-----|
| **不包含** | capability-evolver（独立技能，可被 本地任务编排 单独编排调用） |
...
**关联**: ISC (智能标准中心) | 本地任务编排 (可控任务调度中心) | CRAS (认知进化伙伴)
