...
function resolveGLMApiKey(options = {}) {
  ...
  for (const cfgPath of candidates) {
    try {
      ...
      const root = cfg?.channels || cfg?.providers || cfg;
      const hit = findApiKey(root);
      if (hit) return hit;
    } catch {}
  }

  throw new Error('未找到智谱 API Key。请设置环境变量 ZHIPU_API_KEY，或在构造函数中传入 { apiKey }，或在 openclaw.json 的 channels/providers 中配置。');
}

function findApiKey(obj) {
  if (!obj || typeof obj !== 'object') return null;

  const keyFields = ['apiKey', 'api_key', 'key', 'token'];
  for (const field of keyFields) {
    const v = obj[field];
    if (typeof v === 'string' && /^([a-zA-Z0-9_-]{8,}|sk-|glm-|zhipu)/.test(v)) {
      return v;
    }
  }

  for (const [k, v] of Object.entries(obj)) {
    if (/glm|zhipu|bigmodel/i.test(k)) {
      if (typeof v === 'string' && v.trim()) return v.trim();
      const nested = findApiKey(v);
...
class GLMASR {
  constructor(options = {}) {
    this.apiKey = resolveGLMApiKey(options);
