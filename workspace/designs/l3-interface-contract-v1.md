...
interface RecognizeContext {
  // ─── 对话片段（必填） ───
  messages: MessageSlice[];    // 待识别的消息列表（通常为最近5min的用户消息）

  // ─── 窗口配置 ───
  window: {
    size_ms:   number;         // 消息采集窗口大小（毫秒），默认: 300000（5min）
    max_messages: number;      // 单次最大消息数，默认: 50
  };

  // ─── 历史上下文（可选，用于IC3/IC4推理） ───
  history?: {
    recent_intents: RecentIntent[];  // 最近N条已识别意图（用于上下文连续性）
    session_topics: string[];        // 当前会话话题列表（慢通道产出）
    user_profile?:  object;          // 用户偏好画像（如有）
  };

  // ─── 引擎配置 ───
  config?: {
    model:            "opus" | "glm-5" | "auto";  // LLM选择，默认: auto（先GLM-5，低置信度escalate到Opus）
    min_confidence:   number;    // 最低输出置信度，低于此值不返回，默认: 0.5
    enable_ic3_flag:  boolean;   // 是否标记IC3复杂意图，默认: true
    enable_ic5_split: boolean;   // 是否执行多意图拆分，默认: true
    max_intents_per_message: number;  // 单消息最大意图数，默认: 5
  };
}
...
interface MessageSlice {
  id:              string;    // 消息ID
  content:         string;    // 消息全文
  content_excerpt: string;    // 消息摘要（≤200字符）
  role:            "user" | "assistant" | "system";  // 消息角色
  timestamp:       number;    // Unix毫秒时间戳
  channel?:        string;    // 消息来源通道（feishu / discord / whatsapp）
}
...
```json
{
  ...
  "intents": [
    {
      "intent_id":             "string  — 唯一标识，snake_case，如 file_request",
      ...
      "counter_examples":      ["string — 反例，防止误分类"],

      "mapped_event":          "string  — 识别命中后emit的事件类型（noun.verb格式）",
      "mapped_rules":          ["string — 对应的ISC规则ID列表"],
      ...
      "status_changed_at":     "string  — ISO-8601时间戳",

      "created":               "string  — 创建日期 YYYY-MM-DD",
      "last_triggered":        "string  — 最后触发日期 YYYY-MM-DD",
      "trigger_count_30d":     "number  — 30天触发计数（CRAS回写）",

      "owner":                 "string  — 维护方标识（通常为 aeo）",

      "version_history": [
        {
          "version":   "string  — 变更版本号 v1/v2/v3...",
          "date":      "string  — ISO-8601",
          "change":    "string  — 变更摘要",
          "author":    "string  — 变更人/系统"
        }
      ]
    }
  ],
...
| 字段 | 必填 | 校验 |
|------|------|------|
| `intent_id` | ✅ | `/^[a-z][a-z0-9_]{1,48}$/`，全局唯一 |
| `category` | ✅ | 枚举 IC1-IC5 |
| `display_name` | ✅ | 非空，≤64字符 |
| `description` | ✅ | 非空，≤512字符 |
| `examples` | ✅ | 数组，≥2条，每条≤200字符 |
| `counter_examples` | 推荐 | 数组，建议≥1条 |
| `mapped_event` | ✅ | 必须符合 noun.verb 语法 |
| `mapped_rules` | 推荐 | ISC规则ID数组 |
...
每行内容:
{
  ...
  "recognize_context_hash": "输入上下文的哈希（用于去重和回溯）",
...
| 变更类型 | 示例 | 影响范围 | 审批级别 |
|---------|------|---------|---------|
| **Breaking** | Event Schema增删必填字段、错误码语义变更 | 所有消费方 | 凌霄阁审议 |
...
> **签署确认**：本契约经凌霄阁Day 1审议通过后生效。所有L1/L2/L3的实现必须严格遵守本契约定义的接口签名、数据格式和依赖方向。违反依赖方向的代码变更将被CI门禁自动拒绝。
