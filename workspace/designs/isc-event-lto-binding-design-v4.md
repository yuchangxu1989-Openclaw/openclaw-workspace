...
aeo.*, infra.*, interaction.*, orchestration.*, lto.*, system.*
...
| R08 | 超时重试 | `lto.task.failed` | — | — | — | `system.failure_pattern.pattern_emerged`（反复超时） |
...
| R55 | 自定义决策 | `lto.task.failed` | — | — | — | `system.failure_pattern.pattern_emerged` |
...
| # | 规则 | L1事件 | L2事件 | L3事件 | L4事件 | L5事件 |
|---|------|--------|--------|--------|--------|--------|
| R59 | 根因分析 | `lto.task.failed`, `pipeline.failed` | — | `user.sentiment.frustration.shifted` | `knowledge.debugging_technique.discovered` | `system.failure_pattern.pattern_emerged` ★核心 |
...
class KnowledgeDiscoveryProbe {
  ...
  assessRelevance(finding) {
    ...
    const systemKeywords = ['isc', 'lto', 'event', 'skill', 'agent', 'cras', 'aeo', 
...
| ES05 | lto-event-bus | (废弃) | `lto-core/core/event-bus.js` | ⚠️待废弃 | 统一到ES03 |
...
事件路由结果:
  ...
  "lto.task.failed"                    → failure-handler     (后缀通配)
...
**文档**: isc-event-lto-binding-design-v4
