...
class SkillEvaluator {
    ...
    async evaluateCreate(task) {
        ...
        try {
            const raw = await (0, llm_call_1.callLLMWithFallback)(chain, prompt, this.ctx.log, "SkillEvaluator.create");
    ...
    async evaluateUpgrade(task, skill, skillContent) {
        ...
        const prompt = UPGRADE_EVAL_PROMPT
            ...
            .replace("{SUMMARY}", task.summary.slice(0, 3000));
        try {
            const raw = await (0, llm_call_1.callLLMWithFallback)(chain, prompt, this.ctx.log, "SkillEvaluator.upgrade");
            return this.parseJSON(raw, {
