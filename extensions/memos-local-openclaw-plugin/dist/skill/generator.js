...
class SkillGenerator {
    ...
    async step1GenerateSkillMd(task, conversationText, evalResult) {
        ...
        const prompt = STEP1_SKILL_MD_PROMPT
            ...
            + langInstruction;
        const raw = await (0, llm_call_1.callLLMWithFallback)(chain, prompt, this.ctx.log, "SkillGenerator.step1", { maxTokens: 6000, temperature: 0.2, timeoutMs: 120_000 });
    ...
    async step2ExtractScripts(skillContent, conversationText) {
        ...
        try {
            const raw = await (0, llm_call_1.callLLMWithFallback)(chain, prompt, this.ctx.log, "SkillGenerator.scripts", { maxTokens: 3000, temperature: 0.1, timeoutMs: 120_000 });
    ...
    async step2bExtractReferences(skillContent, conversationText) {
        ...
        const prompt = STEP2B_REFS_PROMPT
            ...
            .replace("{CONVERSATION}", conversationText.slice(0, 6000));
        try {
            const raw = await (0, llm_call_1.callLLMWithFallback)(chain, prompt, this.ctx.log, "SkillGenerator.refs", { maxTokens: 3000, temperature: 0.1, timeoutMs: 120_000 });
            return this.parseJSONArray(raw);
        }
        catch (err) {
    ...
    async step3GenerateEvals(skillContent) {
        ...
        try {
            const raw = await (0, llm_call_1.callLLMWithFallback)(chain, prompt, this.ctx.log, "SkillGenerator.evals", { maxTokens: 2000, temperature: 0.3, timeoutMs: 120_000 });
            return this.parseJSONArray(raw);
        }
        catch (err) {
